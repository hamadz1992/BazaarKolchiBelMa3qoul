...
export default function BarcodeView() {
  ...
  return <div className="barcodeView" dir="rtl">
    <div className="barcodeHeader"><div><h1><ScanBarcode /> الباركود</h1><p>عرض وإدارة باركود السلع</p></div></div>
    <div className="barcodeLayout">
      ...
      <section className="barcodePanel previewPanel">
        <div className="barcodePanelHead"><h2>معاينة الباركود</h2><div className="barcodeActions"><button onClick={copy} title="نسخ الباركود"><Copy size={17}/> نسخ</button><button onClick={() => window.print()} className="primaryAction" title="طباعة"><Printer size={17}/> طباعة</button></div></div>
        <div className="barcodeCard"><strong>{selected.name}</strong><BarcodeSvg value={selected.barcode}/><div className="barcodePrice">{selected.price.toFixed(2)} دج</div></div>
        <p className="barcodeHint">الباركود مرتبط بالسلعة المحددة ولا يتم تغييره عند تعديل بيانات السلعة.</p>
      </section>
    </div>
  </div>;
}
